...
const checkSameRoom = (message) => {
    if (!message.member.voice.channel) return message.reply('Bạn phải vào room voice để dùng lệnh này !');
    if (!message.guild.me.voice.channelID || message.guild.me.voice.channelID == message.member.voice.channelID) return;
    return message.reply('Bạn phải cùng chung phòng với bot để sử dụng lệnh này !');
}
module.exports = {
    checkSameRoom, 
}